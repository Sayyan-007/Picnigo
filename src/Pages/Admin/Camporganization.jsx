...
const Camporganization = () => {
    ...
    const display = async () => {
        try {
            const { data } = await Axios.get(`admin/campdata`)
            console.log(data.campdatas);
            setdisplaydata(data.campdatas)
        }
        catch (error) {
    ...
    const display2 = async () => {
        try {
            ...
            console.log(data);
            const hotel = data.Amount.find((data) => data._id == "Camp")
            setbookingcountdata(hotel.count)
            setrevnew(hotel.totalPackageAmount)
            const profit = data.Persentage.find((data) => data.host_type == "Camp")
    ...
    const display3 = async () => {
        try {
            const { data } = await Axios.get(`admin/camp/Chart`)
            setBarchart(data.Campbooked)
    ...
    const handledisable = async (id) => {
        try {
            const { data } = await Axios.patch(`admin/camp/able`, { isBlocked: true }, { params: { id } })
            console.log(data);
            setdisplaydata((pre) => pre.map((pkg) => pkg._id === id ? { ...pkg, isBlocked: true } : pkg));
    ...
    const handleEnable = async (id) => {
        try {
            const { data } = await Axios.patch(`admin/camp/able`, { isBlocked: false }, { params: { id } })
            console.log(data);
            setdisplaydata((pre) => pre.map((pkg) => pkg._id === id ? { ...pkg, isBlocked: false } : pkg));
    ...
    const option1 = {
        ...
        scales: {
            ...
            {
                title: {
                    display: true, text: "Organization Name",
                        ...
                        <div className="col-lg-3 col-md-6 col-ms-6 mt-4 d-flex">
                            <div className="card flex-fill  p-3" style={{ backgroundColor: "#2A2A3A", color: "#E0E0E0" }}>
                                <div className="d-flex justify-content-center align-items-center">
                                    ...
                                    <h1 className="m-0">{displaydata.length}</h1>
                                </div>
                                <p className="m-0">Oraganization</p>
                            </div>
                        </div>
                        <div className="col-lg-3 col-md-6 col-ms-6 mt-4 d-flex">
                            <div className="card flex-fill  p-3" style={{ backgroundColor: "#2A2A3A", color: "#E0E0E0" }}>
                                ...
                                <tbody style={{ backgroundColor: "#2E2E40", color: "#E0E0E0" }}>
                                    {displaydata.map((data, index) => (
                                        <tr key={data._id} style={{ backgroundColor: index % 2 === 0 ? "#2E2E40" : "#29293D" }}>
                                            ...
                                            <td>
                                                <Link to={`viewcamppackage/${data._id}/${encodeURIComponent(data.username)}`}><button className="btn btn-outline-success">View Packages</button></Link>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
