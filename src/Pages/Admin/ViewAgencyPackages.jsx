...
const ViewAgencyPackages = () => {
    ...
    const display = async () => {
        try {
            const { data } = await Axios.get(`admin/agency/display`, { params: { id } })
    ...
    const handlereject = async (id) => {
        try {
            const { data } = await Axios.patch(`admin/agency/packageupdate`, { Status: "reject", Disabled: true }, { params: { id } })
            console.log(data);
    ...
    const handleaccept = async (id) => {
        try {
            const { data } = await Axios.patch(`admin/agency/packageupdate`, { Status: "accept" }, { params: { id } })
            console.log(data);
            toast.success(data.message)
                ...
                <div className="col-md-9">
                    ...
                    <div className="row justify-content-center text-center mt-4">
                        <div className="col-md-6">
                            <h4 className="fw-light">Agency ID :- {id}</h4>
                            <h4 className="fw-light">Agency Name :- {name}</h4>
                        </div>
                    </div>
                            ...
                            {displaydata.filter(data => data.Status === "accept").map((data, index) => (
                                <div key={data._id} className="col-lg-5 col-md-5 m-2  card shadow p-4" style={{backgroundColor:"#2D2D44",color: "#E0E0E0" }}>
                                    <div>
                                        ...
                                        <p>ID :- {data._id}</p>
                                        <h5 className="p-0">Destination :- {data.Destination_Name}</h5>
                                    </div>
