...
const ViewOrganizationpackages=()=>{
    ...
    const display=async()=>{
        try 
        {
            const {data}= await Axios.get(`admin/camp/display`,{params:{id}})
    ...
    const handlereject=async(id)=>{
        try 
        {
            const {data} =  await Axios.patch(`admin/camp/packageupdate`,{Status : "reject",Disabled:true},{params:{id}}) 
            console.log(data);
    ...
    const handleaccept=async(id)=>{
        try 
        {
            const {data} =  await Axios.patch(`admin/camp/packageupdate`,{Status : "accept"},{params:{id}}) 
            console.log(data);
            toast.success(data.message)
        ...
        <div className="container-fluid" style={{ backgroundColor: "#1E1E2F",color: "#E0E0E0" }}>
            <div className="row">
                ...
                <div className="col-md-9">
                    <NavbarAdmin/>
                    <div className="row justify-content-center text-center mt-4 p-0 m-0">
                        <div className="col-md-5">
                            <h4 className="fw-light">Organization ID :- {id}</h4>
                            <h4 className="fw-light">Organization Name :- {name}</h4>
                        </div>
                    </div>
                            ...
                            {displaydata.filter(data => data.Status === "accept").map((data, index) => (
                                <div key={data._id} className="col-lg-5 col-md-5 m-2  card shadow p-4" style={{backgroundColor:"#2D2D44",color: "#E0E0E0" }}>
                                    <div>
                                        ...
                                        <p>ID :- {data._id}</p>
                                        <h5 className="p-0">Destination :- {data.campName}</h5>
                    ...
                    <div className="row mt-4">
                        <h4 className="m-0">Pending Packages({displaydata.filter(data => data.Status === "pending").length})</h4>
                        <div className="mt-3 d-flex flex-sm-row flex-column overflow-scroll webscrollbar">
                    ...
                    <div className="row mt-4">
                        <h4 className="m-0">Rejected Packages({displaydata.filter(data => data.Status === "reject").length})</h4>
                        <div className="mt-3 d-flex flex-sm-row flex-column overflow-scroll webscrollbar">
...
export default ViewOrganizationpackages