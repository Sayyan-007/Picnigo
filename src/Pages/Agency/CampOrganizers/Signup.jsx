...
const CampSignup=()=>{
    ...
    const signup = useFormik({
        ...
        onSubmit:async(values)=>{
            ...
            try 
            {
                const {data} = await Axios.post('/oraganization/signup',values)
                console.log(data);
            ...
            <div  className="min-vh-100 d-flex justify-content-center align-items-center" style={{backgroundImage: `url('/world.jpg')`,backgroundSize: 'cover', backgroundPosition: 'center'}}>
                <div>
                    <form onSubmit={signup.handleSubmit} className="d-flex flex-column w-100 gap-4 gap-sm-4 " style={{color:"#E0E0E0"}}>
                        <h1 className='fw-bolder  fs-1 text-center'>SignUp Camp Oraganization</h1>
                        ...
                        <div className='d-flex flex-column flex-sm-row gap-3 pb-4'>
                            ...
                            <button  className='opp-btn p-2 px-3'><Link className='text-decoration-none text-light' to={'/camporganization/login'}><h6 className='m-0'>Already have an mangment Account</h6></Link></button>
                        </div>
                    </form>
                </div>
            </div>
